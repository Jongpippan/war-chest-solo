...
import { cloneState, executeAction, generateAllMainActions, generatePendingActions, getUnit, unitsOf, } from './engine.js';
function strategicLocationDistance(state, player, hex) {
    const targets = ALL_LOCATIONS.filter((h) => state.locations[h] !== player);
    if (!targets.length)
        return 0;
    return Math.min(...targets.map((h) => distance(hex, h)));
}
function targetValue(state, targetId) {
    const target = targetId ? getUnit(state, targetId) : undefined;
    if (!target)
        return 0;
    let v = target.strength === 1 ? 250 : 110;
    if (Object.prototype.hasOwnProperty.call(state.locations, target.hex))
        v += 70;
    if (target.type === 'WARRIOR_PRIEST' || target.type === 'KNIGHT')
        v += 25;
    return v;
}
function moveValue(state, unitId, destination) {
    ...
    if (Object.prototype.hasOwnProperty.call(state.locations, destination) && state.locations[destination] !== unit.owner)
        score += 75;
    const enemyAdj = state.boardUnits.filter((u) => u.owner !== unit.owner && distance(destination, u.hex) === 1).length;
    score += enemyAdj * 12;
    return score;
}
export function scoreAction(state, action, randomize = true) {
    ...
    switch (action.kind) {
        ...
        case 'RECRUIT': {
            ...
            if (deployed)
                score += 18;
            if (type === 'MERCENARY' && deployed)
                score += 65;
            break;
        }
        case 'DEPLOY': {
            score += 120;
            if (payload.destination && Object.prototype.hasOwnProperty.call(state.locations, payload.destination) && state.locations[payload.destination] !== p)
                score += 40;
        ...
        case 'BOLSTER': {
            ...
            if (unit?.type === 'KNIGHT')
                score += 55;
            if (unit && state.boardUnits.some((u) => u.owner !== p && distance(unit.hex, u.hex) <= 2))
                score += 40;
            break;
        }
        case 'MOVE':
        ...
        case 'FREE_CONTROL': {
            ...
            if (state.players[p].markersRemaining === 1)
                score += 10000;
            break;
        }
        case 'ATTACK':
    ...
    if (target && Object.prototype.hasOwnProperty.call(state.locations, target.hex))
        score += 35;
    return score;
}
function positionScore(state, player) {
    const foe = player === 'human' ? 'bot' : 'human';
...
function hardScore(state, action) {
    ...
    switch (action.kind) {
        ...
        case 'TACTIC_MARSHALL': {
            ...
            if (target?.strength === 1)
                return '한 번의 공격으로 유닛을 제거할 수 있어 교환 가치가 높습니다.';
            if (target && Object.prototype.hasOwnProperty.call(state.locations, target.hex))
                return '거점 위의 적을 약화시키면 다음 점령 싸움이 유리해집니다.';
...
export function chooseBotDecision(state, difficulty = 'NORMAL') {
    ...
    const scored = actions.map((action) => {
        let score;
        if (difficulty === 'HARD')
            score = hardScore(state, action);
        else
            score = scoreAction(state, action, difficulty === 'EASY');
        return { action, score };
