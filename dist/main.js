...
import { afterResolvedAction, cloneState, createGame, executeAction, generateActionsForCoin, generatePendingActions, normalizeUnitIds, stateSanity, } from './engine.js';
...
function difficultyDescription(d) {
    if (d === 'EASY')
        return '좋은 수 주변에서 일부러 흔들립니다. 전술 실수가 섞입니다.';
    if (d === 'HARD')
        return '즉시 점수 + 행동 후 보드 가치까지 평가합니다.';
...
function unitIconSvg(coin, className = 'unit-icon') {
    return `<svg viewBox="0 0 24 24" class="${className}" aria-hidden="true">${unitIconPaths(coin)}</svg>`;
}
function tokenIconMarkup(coin, x, y, size, color = '#fff8e8') {
    const scale = size / 24;
...
function tooltipStatsHtml(stats) {
    const entries = [];
    if (stats.owner)
        entries.push(`<span>${esc(stats.owner)}</span>`);
    if (stats.stack)
        entries.push(`<span>Stack ${esc(stats.stack)}</span>`);
    if (stats.location)
        entries.push(`<span>${esc(stats.location)}</span>`);
    if (stats.supply)
        entries.push(`<span>Supply ${esc(stats.supply)}</span>`);
    if (stats.board)
        entries.push(`<span>Board ${esc(stats.board)}</span>`);
    if (stats.removed)
        entries.push(`<span>Out ${esc(stats.removed)}</span>`);
...
function bindUnitInfoInteractions() {
    ...
    document.querySelectorAll('[data-unit-type]').forEach((node) => {
        node.addEventListener('mouseenter', (evt) => {
            ...
            const coin = (el.dataset.unitType ?? 'ROYAL');
        ...
        node.addEventListener('mousemove', (evt) => positionTooltip(tooltip, evt));
        node.addEventListener('mouseleave', hide);
    });
}
function renderSelectCard(type, selected) {
...
function renderDraftCard(type, mine, theirs, disabled) {
    const d = UNIT_DEFS[type];
...
function renderSetup() {
    ...
    bindDifficultyButtons();
    document.querySelectorAll('.unit-pick[data-unit]').forEach((btn) => btn.addEventListener('click', () => {
...
function tableCoin(coin, options = {}) {
...
function renderControlMarkers(id) {
    if (!state)
        return '';
...
function describeActionAnimation(before, action) {
...
async function executeHumanAction(action) {
...
function renderContextBoardActions(actions) {
    if (!state || boardPath.length !== 1 || !boardPath[0].startsWith('unit:'))
        return '';
...
function renderInteractionHud(actions) {
    ...
    if (state.activePlayer !== 'human')
        return `<div class="interaction-hud bot-turn-hud"><strong>BOT TURN</strong><span>${actionAnimating ? 'Resolving the highlighted action…' : 'The bot is choosing an action…'}</span></div>`;
...
function renderActionPlaybackSvg() {
    if (!actionAnimation)
        return '';
...
function renderActionPlaybackToast() {
    if (!actionAnimation)
        return '';
...
function renderBoardSvg(actions = []) {
    ...
    const ui = boardInteractionState(actions);
    const unitLayer = [];
    const locationOverlayLayer = [];
    const stackBadgeLayer = [];
    const actionChipLayer = [];
...
function humanCandidates() {
    if (!state || actionAnimating || state.activePlayer !== 'human' || state.winner)
        return [];
    if (state.pending)
...
function renderGame() {
    ...
    document.querySelectorAll('[data-recruit-type]').forEach((el) => el.addEventListener('click', () => {
        ...
        if (action)
            void executeHumanAction(action);
    }));
