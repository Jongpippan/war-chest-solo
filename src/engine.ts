...
export const otherPlayer = (id: PlayerId): PlayerId => (id === 'human' ? 'bot' : 'human');
...
function makePlayer(id: PlayerId, units: UnitType[]): PlayerState {
  const supply: Partial<Record<UnitType, number>> = {};
  const bag: Coin[] = ['ROYAL'];
...
export function createGame(humanUnits: UnitType[], botUnits: UnitType[], initiative?: PlayerId): GameState {
  if (humanUnits.length !== 4 || botUnits.length !== 4) throw new Error('Each player must have exactly four unit types.');
  if (new Set(humanUnits).size !== 4 || new Set(botUnits).size !== 4) throw new Error('Unit types must be unique per player.');
...
export function getUnit(state: GameState, unitId: string): BoardUnit | undefined {
  return state.boardUnits.find((u) => u.id === unitId);
}

export function unitAt(state: GameState, hex: HexId): BoardUnit | undefined {
  return state.boardUnits.find((u) => u.hex === hex);
}

export function unitsOf(state: GameState, player: PlayerId, type?: UnitType): BoardUnit[] {
  return state.boardUnits.filter((u) => u.owner === player && (!type || u.type === type));
}

export function isEmpty(state: GameState, hex: HexId): boolean {
  return !unitAt(state, hex);
}

export function drawOne(state: GameState, playerId: PlayerId): Coin | null {
...
export function drawRoundHands(state: GameState): void {
...
function discardCoin(state: GameState, playerId: PlayerId, coin: Coin, faceUp: boolean): void {
...
function consumeCandidateCoin(state: GameState, candidate: ActionCandidate, destination: 'DISCARD_UP' | 'DISCARD_DOWN' | 'BOARD' | 'NONE'): Coin | null {
  ...
  if (destination === 'DISCARD_DOWN') discardCoin(state, candidate.player, coin, false);
  return coin;
}
...
function candidate(
  ...
  label: string,
  group: string,
  payload: ActionCandidate['payload'],
  relatedHexes: HexId[] = [],
  coin?: Coin,
  coinIndex?: number,
): ActionCandidate {
...
function deployedLimit(type: UnitType): number {
  return type === 'FOOTMAN' ? 2 : 1;
}

function normalMoveDestinations(state: GameState, unit: BoardUnit): HexId[] {
  return neighbors(unit.hex).filter((h) => isEmpty(state, h));
}

function normalAttackTargets(state: GameState, attacker: BoardUnit): BoardUnit[] {
  if (attacker.type === 'ARCHER' || attacker.type === 'LANCER') return [];
  return state.boardUnits.filter((target) => {
    if (target.owner === attacker.owner) return false;
    if (distance(attacker.hex, target.hex) !== 1) return false;
    if (target.type === 'KNIGHT' && attacker.strength < 2) return false;
    return true;
  });
}

function canAttackAfterMoving(attacker: BoardUnit, target: BoardUnit, attackerHex: HexId): boolean {
  if (target.owner === attacker.owner) return false;
  if (distance(attackerHex, target.hex) !== 1) return false;
  if (target.type === 'KNIGHT' && attacker.strength < 2) return false;
  return true;
}

function deployHexes(state: GameState, playerId: PlayerId, type: UnitType): HexId[] {
  const ownedLocations = ALL_LOCATIONS.filter((h) => state.locations[h] === playerId && isEmpty(state, h));
  if (type !== 'SCOUT') return ownedLocations;
  const scoutExtras = new Set<HexId>();
  for (const friendly of unitsOf(state, playerId)) {
    for (const h of neighbors(friendly.hex)) if (isEmpty(state, h)) scoutExtras.add(h);
  }
  return [...new Set([...ownedLocations, ...scoutExtras])];
}

function makeBaseCandidates(
  state: GameState,
  playerId: PlayerId,
  coin: Coin,
  source: ActionSource,
  ...
  if (type === 'ARCHER') {
    for (const archer of matching) {
      for (const target of state.boardUnits.filter((u) => u.owner !== playerId && distance(archer.hex, u.hex) === 2)) {
        if (target.type === 'KNIGHT' && archer.strength < 2) continue;
  ...
  if (type === 'CROSSBOWMAN') {
    for (const crossbow of matching) {
      for (let dir = 0; dir < 6; dir += 1) {
        ...
        if (!mid || !targetHex || !isEmpty(state, mid)) continue;
        const target = unitAt(state, targetHex);
        if (!target || target.owner === playerId) continue;
        if (target.type === 'KNIGHT' && crossbow.strength < 2) continue;
  ...
  if (type === 'LANCER') {
    for (const lancer of matching) {
      for (let dir = 0; dir < 6; dir += 1) {
        ...
        if (!move1 || !isEmpty(state, move1)) continue;
        ...
        if (!move2 || !isEmpty(state, move2)) continue;
        const targetAfter2Hex = stepInDirection(lancer.hex, dir, 3);
        if (!targetAfter2Hex) continue;
    ...
    for (const light of matching) {
      ...
      for (const mid of normalMoveDestinations(state, light)) {
        for (const dest of neighbors(mid)) {
          if (dest === light.hex || !isEmpty(state, dest) || seen.has(dest)) continue;
          seen.add(dest);
...
function royalGuardTacticCandidates(
  state: GameState,
  playerId: PlayerId,
  source: ActionSource,
  coinIndex?: number,
  ...
  for (const guard of unitsOf(state, playerId, 'ROYAL_GUARD')) {
    ...
    for (const one of normalMoveDestinations(state, guard)) {
      ...
      for (const two of neighbors(one)) {
        if (two === guard.hex || !isEmpty(state, two) || seen.has(two)) continue;
        if (state.locations[two] !== playerId) continue;
        seen.add(two);
...
export function generateActionsForCoin(
  state: GameState,
  playerId: PlayerId,
  coin: Coin,
  source: ActionSource,
...
function freeManeuverCandidates(state: GameState, playerId: PlayerId, unit: BoardUnit, includeSkip: boolean, labelPrefix: string): ActionCandidate[] {
  const out: ActionCandidate[] = [];
  for (const dest of normalMoveDestinations(state, unit)) {
  ...
  if (includeSkip) out.push(candidate(playerId, 'FREE', 'SKIP_ABILITY', `${labelPrefix}: 사용하지 않음`, '특수 능력', { unitId: unit.id }, [unit.hex]));
  return out;
}

export function generatePendingActions(state: GameState): ActionCandidate[] {
...
function removeBoardCoin(state: GameState, unit: BoardUnit): void {
...
function royalGuardAbsorbsFromSupply(state: GameState, target: BoardUnit): boolean {
  if (target.type !== 'ROYAL_GUARD') return false;
  const supply = state.players[target.owner].supply.ROYAL_GUARD ?? 0;
  if (supply <= 0) return false;
  if (target.owner === 'bot') return true;
...
function applyControl(state: GameState, playerId: PlayerId, unit: BoardUnit): void {
  const current = state.locations[unit.hex];
  if (current === undefined || current === playerId) throw new Error('Illegal control action.');
...
function resolveRecruit(state: GameState, playerId: PlayerId, recruitType: UnitType): void {
  const player = state.players[playerId];
  if ((player.supply[recruitType] ?? 0) <= 0) throw new Error('No supply coin.');
  ...
  if (recruitType === 'MERCENARY') {
    ...
    if (merc) state.pending = { kind: 'MERCENARY_EXTRA', player: playerId, unitId: merc.id };
  }
}

function ensureUnit(state: GameState, id?: string): BoardUnit {
  if (!id) throw new Error('Missing unit id.');
  const unit = getUnit(state, id);
  if (!unit) throw new Error('Unit no longer exists.');
  return unit;
}

function advanceFootmanPending(state: GameState): void {
  if (!state.pending || state.pending.kind !== 'FOOTMAN_QUEUE') return;
  const nextIndex = state.pending.index + 1;
  if (nextIndex >= state.pending.unitIds.length) state.pending = null;
  else state.pending = { ...state.pending, index: nextIndex };
}
...
export function executeAction(state: GameState, action: ActionCandidate): void {
  ...
  switch (action.kind) {
    ...
    case 'MOVE':
    case 'FREE_MOVE': {
      if (action.kind === 'MOVE') consumeCandidateCoin(state, action, 'DISCARD_UP');
      const unit = ensureUnit(state, action.payload.unitId);
    ...
    case 'ATTACK':
    case 'FREE_ATTACK': {
      if (action.kind === 'ATTACK') consumeCandidateCoin(state, action, 'DISCARD_UP');
    ...
    case 'CONTROL':
    case 'FREE_CONTROL': {
      if (action.kind === 'CONTROL') consumeCandidateCoin(state, action, 'DISCARD_UP');
      const unit = ensureUnit(state, action.payload.unitId);
      const unitType = unit.type;
      applyControl(state, playerId, unit);
      if (!state.winner) setPostManeuverTriggerWithType(state, playerId, unit.id, unitType, 'CONTROL');
      break;
    }
    case 'TACTIC_ARCHER': {
...
export function afterResolvedAction(state: GameState): void {
  if (state.winner || state.pending || state.forcedCoin) return;
...
export function isMainActionPhase(state: GameState): boolean {
  return !state.pending && !state.winner;
}

export function actionCountForPlayer(state: GameState, playerId: PlayerId): number {
  if (state.pending) return state.pending.player === playerId ? generatePendingActions(state).length : 0;
  return generateAllMainActions(state, playerId).length;
}

export function totalKnownCoins(player: PlayerState, type: Coin): number {
...
export function availableUnitTypes(): UnitType[] {
  return [...ALL_UNITS];
}

export function startingHexes(player: PlayerId): HexId[] {
